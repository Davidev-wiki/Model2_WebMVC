		...
		if (isudType !=null && isudType.length() > 0){
			...
			if ("SALL".equals(isudType)){
				...
				if (aListAll !=null && aListAll.size() > 0) {
					...
					out.println("<script>");					
					out.println("location.href='/testKosmo/mem?isudType=SALL'");
					out.println("</script>");
				}		
			} // 전체 회원정보 조회 end
			...
			if ("UOK".equals(isudType)){
				System.out.println("회원 정보 수정 isudType >>> : " + isudType);
				...
				if (mnum !=null && mnum.length() > 0){
					...
					mvo.setMnum(mnum);
					mvo.setMemail(memail);
					boolean update = ms.memberUpdate(mvo);
					
					if (update != false) {
						System.out.println("update 성공여부 >>> : " + update);						
						request.setAttribute("update", update);					
						RequestDispatcher rd= request.getRequestDispatcher("/kosmo/mem/memSeleAll.jsp");
						rd.forward(request, response);
			...
			if ("DOK".equals(isudType)){
				...
				if (mnum !=null && mnum.length() > 0){
					...
					if (delete != false) {
						System.out.println("delete 성공여부 >>> : " + delete);						
